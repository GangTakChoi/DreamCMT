...
<form name="comment_form" action="./delete_comment.php" method="POST" >
<input type="submit" value="선택된 댓글 삭제"></input>
<input type='button' onclick='comment_check_all();' value='모두 선택' />
<input type='button' onclick='comment_uncheck_all();' value='모두 해제' />
<?php
if(empty($page))
        $page=1;
$value = ($page-1)*20;
...
?>
	...
	<table style="box-shadow:5px 5px 10px #cccccc;width:850px;padding:0px 0px 0px 0px;border:1px solid #EAE9FF;margin-bottom:8px">
		<tr>
			<td rowspan=3 style="width:20px"><input type="checkbox" id="comment_seq" name="comment_seq[]" value="<?php echo $comment_data['seq']."/".$comment_data['category']?>"></td>
